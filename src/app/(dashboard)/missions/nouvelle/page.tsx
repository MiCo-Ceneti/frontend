...
import type { Utilisateur, TypeMission, Paginated } from "@/lib/types";
...
export default function NouvelleMissionPage() {
  ...
  React.useEffect(() => {
    api.get<Paginated<Utilisateur>>("utilisateurs/", { actif: true }).then((d) => setAgents(d.results));
    api.get<Paginated<TypeMission>>("types-mission/").then((d) => setTypesMission(d.results));
  }, []);
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setEnCours(true);
    try {
      const mission = await api.post<{ id: string }>("missions/", {
        ...form,
        ...
        setErreur(
          typeof detail?.detail === "string"
            ...
            : Array.isArray(detail?.non_field_errors)
              ...
              : "Impossible de creer la mission. Verifiez les champs."
        );
      } else {
  ...
  return (
    <div>
      <PageHeader title="Nouvelle mission" description="Designer un ou plusieurs agents pour une mission" />
        ...
        <Card>
          <CardContent className="flex flex-col gap-3 p-5">
            <div className="flex items-center justify-between">
              ...
              <span className="font-mono text-xs text-muted-foreground">{agentsSelectionnes.length}</span>
            </div>
              ...
              <div className="flex flex-col gap-1 p-2">
                {agents.map((agent) => (
                  ...
                  >
                    ...
                    />
                    <span className="flex-1">{agent.nom_complet}</span>
                    <span className="font-mono text-xs text-muted-foreground">{agent.matricule}</span>
                  </label>
                ))}
              </div>
            </ScrollArea>
